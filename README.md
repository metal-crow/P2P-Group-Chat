...
The Third stage, the decentralization, is when multiple users are connected to the chat, and the host user terminates. If this occurs, another user must be chosen to be the host, and run the connection listener and relay receiver threads. Every other user must also know who the backup host will be, because they will need to connect to their IP.
In the main file, whenever the user starts the program it either creates a server or connects to it, depending on the user’s input. If the host exits, then the listener receiver gets an error, which we can tell it it’s unexpected by checking a Boolean in the main file that keeps track of the users connection state. (This error is generated whenever the socket closes, but if we purposefully close the socket we change the Boolean value and know that this error is acceptable. I know this is weird, but it works.) If this error is thrown and the Boolean is unchanged, we can conclude that the connection was terminated by the host’s disconnection, and need to recreate the host.
Before any of this happens, while the original host is still up, whenever anyone connects to the original host the host generates a random number corresponding to a connected person (excluding the host). That user and all other users are informed of who will be the backup host and what their ip is. This is used when the host exits.
The main file has been running in a loop while the program is running, but doesn’t do anything because once the user connects a Boolean “connected” is set to true, and not more code is executed in the while loop. When the host disconnects, we detect this and set the “connected” variable back to false. Additionally, we set the host ip address variable to the emergency host’s ip address, and the variable that the user previous set that determined whether they would host or connect to either connect of host automatically, depending if they were chosen to be the emergency host or not. Now, the main file’s loop re-runs the code in the if statement blocked off by the if statement !connected. This code is the same as when the user starts the program, except that now they variable determining connecting or hosting is automatically set, and it either ONLY tries to connect to the server stored in the host variable, which is now the emergency host’s, or hosts the server. This effectively starts the program off on a blank slate, with only the information stored in variables kept, and everyone seamlessly connects to the new emergency host and the process repeats.
