...
In the main file, whenever the user starts the program it either creates a server or connects to it, depending on the user. If the host exits, then the listener receiver gets an error, which we can tell it it’s unexpected by checking a Boolean in the main file that keeps track of the users connection state. (This error is generated whenever the socket closes, but if we purposefully close the socket we change the Boolean value and know that this error is acceptable. I know this is weird, but it works.) If this error is thrown and the Boolean is unchanged, we can conclude that the connection was terminated by the host’s disconnection, and need to recreate the host.
Before any of this happens, while the original host is still up, whenever anyone connects to the original hosts generates a random number corresponding to a connected person (excluding the host) is generated and that user and all other users are informed of who will be the backup host and what their ip is. This is used when the host exits.
The main file has been running in a loop while the program is running, but doesn’t do anything because once the user connects a Boolean “connected” is set to true, and not more code is executed in the while loop. When the host disconnects, we detect this and set the “connected” variable back to false, as well as setting the host ip address variable to the emergency host’s ip address, causing the main file’s loop to re-run the code in the if statement blocked off by the if statement !connected. This code is the same as when the user starts the program, except that is ONLY tries to connect to the server stored in the host variable, which is now the emergency host’s. This effectively starts the program off on a blank slate, with only the information stored in variables kept, and everyone seamlessly connects to the new emergency host and the process repeats.
